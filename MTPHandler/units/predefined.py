...
class Unit:
    ...
    @staticmethod
    def metre():
        return BaseUnit(kind=UnitType.METRE, exponent=1, scale=1)
...
metre = UnitDefinition(base_units=[Unit.metre()])._get_name()
nm = UnitDefinition(base_units=[n * Unit.metre()])._get_name()

## Ontology

metre.ld_id = ONTOMAPS["length"]["metre"]
nm.ld_id = ONTOMAPS["length"]["nm"]
